...
if (DEBUG_CONDITION){
  ...
  for (int index = 0; index < partners_size; index++) {
    if (sem("reduction")) {
      // std::cout << "in reduction "<< index << std::endl;
      ...
      //std::cout << blockID << ",\t"<<m.GetMpiRankFromId(blockID) << std::endl;
    }
  ...
  if (sem("domain_overwrite")) {
    if (cag.size() != 0){
      std::cout << "CAG of block "<<std::to_string(blockID)<<" is not empty in the end! Switch to different labeling method!"<< std::endl;
      assert(cag.size() != 0);
    }else{
      for (auto l : layers) {
        for (auto c : m.Cells()) {
